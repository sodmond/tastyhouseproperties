                ...
                <div class="dashboard-right-sidebar">
                    <div class="product-tab">
                        ...
                        <div class="table-responsive dashboard-bg-box">
                            <div class="dashboard-title dashboard-flex" style="margin-bottom:10px">
                                <h3>{{ $chat->seller->companyname ?? $chat->seller->firstname.' '.$chat->seller->lastname }}</h3>
                                <button class="btn btn-sm theme-bg-color"><a class="text-white" href="{{ route('user.messages') }}"><i class="fa fa-arrow-left"></i>  Back</a></button>
                            </div>
...
<input type="hidden" id="sendMessageLink" value="{{ route('user.message.send', ['id' => $chat->id]) }}">
@endsection
    ...
    <script>
        $(document).ready(function(){
            ...
            window.Echo.channel(`chat`).listen('.chat.sent', (e) => {
                ...
                if(e.user_id == userId && e.seller_id == sellerId && e.sender == 'seller' && e.id == chatId) {
